...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Fungsi untuk menyorot tombol yang ditekan
  function highlightKey(key) {
    keys.forEach(element => {
      if (element.textContent === key) {
        element.classList.add('key--clicked'); // Tambahkan kelas tanpa menghapusnya
      }
    });
  }
// Array untuk menyimpan riwayat tombol yang ditekan
let keyHistory = [];

// Fungsi untuk menangani event penekanan tombol
function handleKeyPress(event) {
  // Menyimpan tombol yang ditekan ke dalam array
  keyHistory.push(event.key);

  // Menampilkan riwayat tombol yang ditekan (opsional)
  updateKeyHistoryDisplay();

  // Anda juga bisa membatasi jumlah riwayat yang disimpan
  if (keyHistory.length > 10) {
    keyHistory.shift(); // Menghapus tombol tertua jika riwayat melebihi 10
  }
}

// Fungsi untuk memperbarui tampilan riwayat tombol yang ditekan
function updateKeyHistoryDisplay() {
  const historyDisplay = document.getElementById('key-history-display');
  if (historyDisplay) {
    historyDisplay.textContent = keyHistory.join(', ');
  }
}
...
document.addEventListener('keydown', handleKeyPress);
  ...
  document.getElementById('reset-button').addEventListener('click', function () {
    keys.forEach(key => {
      key.classList.remove('key--clicked'); // Hapus kelas .key--clicked
    });
  });
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function activateTheme(theme) {
    ...
    body.classList.remove("theme--retro", "theme--navy-blue");

    // Tambahkan class tema yang dipilih
    body.classList.add(`theme--${theme}`);

    // Simpan tema yang dipilih ke localStorage (opsional)
    localStorage.setItem("selectedTheme", theme);

    // Update tampilan lingkaran tema
  ...
  // Event listener untuk setiap lingkaran tema
  themeOptions.forEach((option) => {
  ...
  // Cek localStorage untuk tema yang sudah dipilih sebelumnya
  const savedTheme = localStorage.getItem("selectedTheme");
  if (savedTheme) {
    activateTheme(savedTheme);
  } else {
    // Set tema default jika tidak ada tema yang disimpan
    activateTheme("retro");
  }
});
